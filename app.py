...
# PATCH-25: Emotional departure & companion follow thresholds
EMOTIONAL_DEPARTURE_TENSION_THRESHOLD = 70    # 긴장 이 이상이면 회피 판정
EMOTIONAL_DEPARTURE_AFFECTION_THRESHOLD = 30  # 호감 이 이하이면 회피 판정
EMOTIONAL_DEPARTURE_PROBABILITY = 0.4         # 감정적 회피 확률
STRESS_DEPARTURE_THRESHOLD = 80               # 스트레스 이 이상이면 퇴장 판정
STRESS_DEPARTURE_PROBABILITY = 0.25           # 스트레스 퇴장 확률
COMPANION_FOLLOW_AFFECTION_THRESHOLD = 40     # 동행 따라가기 호감 임계
COMPANION_FOLLOW_BASE_PROBABILITY = 0.6       # 호감 낮아도 따라갈 기본 확률
PROFILE_CONFIDENCE_MAX = 100                  # 프로파일 확신도 최대값
PROFILE_CONFIDENCE_MULTIPLIER = 15            # 데이터 포인트당 확신도 증가
...
def check_emotional_departure(s: dict) -> list:
    ...
    for char_name in list(s.get("on_screen", [])):
        ...
        # 판정 조건: 긴장 높고 호감 낮으면 → 회피 가능성
        if tension >= EMOTIONAL_DEPARTURE_TENSION_THRESHOLD and affection <= EMOTIONAL_DEPARTURE_AFFECTION_THRESHOLD:
            if random.random() < EMOTIONAL_DEPARTURE_PROBABILITY:
                move_character(s, char_name, "???", "감정적 회피")
                departed.append(char_name)
        # 스트레스 높으면 → 혼자 있고 싶음
        elif stress >= STRESS_DEPARTURE_THRESHOLD:
...
def analyze_player_profile(s: dict, user_input: str):
    ...
    profile["confidence_score"] = min(PROFILE_CONFIDENCE_MAX, data_points * PROFILE_CONFIDENCE_MULTIPLIER)
        ...
        if new_loc:
            ...
            for char_name in list(s.get("on_screen", [])):
                ...
                if char_loc == player_loc:  # 이전에 같은 곳에 있었으면
                    rel = s.get("relationships", {}).get(char_name, {})
                    affection = rel.get("affection", 50)
                    if affection >= COMPANION_FOLLOW_AFFECTION_THRESHOLD or random.random() < COMPANION_FOLLOW_BASE_PROBABILITY:
